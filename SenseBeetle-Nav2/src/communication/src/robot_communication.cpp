...
RobotCommunicationNode::RobotCommunicationNode(
  ...
  costmap_sub_ = this->create_subscription<nav_msgs::msg::OccupancyGrid>(
    "global_costmap/costmap", 5,
    std::bind(&RobotCommunicationNode::CostmapCallBack, this,
              std::placeholders::_1));
  ...
  image_sub_ = this->create_subscription<sensor_msgs::msg::Image>(
    "camera/camera/depth/color/image_raw", 2,
    std::bind(&RobotCommunicationNode::ImageCallBack, this,
              std::placeholders::_1));

  goal_point_pub_ =
    this->create_publisher<geometry_msgs::msg::PoseStamped>("goal_pose", 2);
...
void RobotCommunicationNode::WayPointCallBack(
  ...
  goal_point_pub_->publish(local_pose);
}
...
void RobotCommunicationNode::CostmapCallBack(
  const nav_msgs::msg::OccupancyGrid::ConstSharedPtr costmap_msg) {
  nav_msgs::msg::OccupancyGrid totalCostmap;
  totalCostmap = *costmap_msg;
  totalCostmap.header.stamp = costmap_msg->header.stamp;
  totalCostmap.header.frame_id = MAP_FRAME;

  std::vector<uint8_t> data_buffer =
    SerializeMsg<nav_msgs::msg::OccupancyGrid>(totalCostmap);
...
void RobotCommunicationNode::TFUpdateThread() {
  while (rclcpp::ok()) {
    ...
    transformStamped.child_frame_id =
      "robot_" + std::to_string(robot_id) + "/" + BASE_FRAME;
